package services

import (
	"encoding/json"
	"errors"
	"github.com/astaxie/beego/logs"
	"github.com/astaxie/beego/validation"
	"openai-backend/models"
)

/*
@Author:
@Time: 2020-11-05 15:02
@Description:abilityCategoryService
*/

//新建abilityCategory数据校验
func ValidatePostAbilityCategory(requestBody []byte) (*models.AbilityCategoryModel, error) {
	//获取requestBody中的参数
	type requestJSON struct {
		CategoryName     string `description:"能力种类名称" json:"categoryName"`
	}
	var jSON requestJSON

	if err := json.Unmarshal(requestBody, &jSON); err != nil {
		logs.Error("err=", err)
		return nil, errors.New("json 格式错误" + err.Error())
	}
	//校验数据
	valid := validation.Validation{}
	valid.Required(jSON.CategoryName, "categoryName").Message("未输入名称 ")

	if valid.HasErrors() {
		var errStr string
		for _, err := range valid.Errors {
			errStr = errStr + err.Message
		}
		//	logs.Error("err", errStr)
		return nil, errors.New(errStr)

	}
	//将json数据复制给model
	model := &models.AbilityCategoryModel{
		CategoryName:     jSON.CategoryName,

	}

	if model.QueryTable().Filter("category_name", model.CategoryName).Exist() == true {
		return nil, errors.New("名称重复")
	}

	return model, nil
}





